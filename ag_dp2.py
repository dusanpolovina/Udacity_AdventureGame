...
def forked_path(items, villain_choice):
    ...
    if "1" in choice1 or "door" in choice1 or "tower" in choice1 or "left" in choice1:
...
def item_swap(items, villain_choice, choice5, number, name):
    if number in choice5 or name in choice5:
        if name in items:
            print_pause("You already have that item")
            item_choices(items, villain_choice)
        elif name not in items and len(items) == 1:
            items.append("axe")
            print_pause("You pick up the " + items[1] + "")
            item_choices(items, villain_choice)
        else:
            print_pause("You drop the " + items[1] + "")
...
def item_choices(items, villain_choice):
    ...
    if "1" in choice5 or "axe" in choice5:
        if "axe" in items:
            print_pause("You already have that item")
            item_choices(items, villain_choice)
        elif "axe" not in items and len(items) == 1:
            items.append("axe")
            print_pause("You pick up the " + items[1] + "")
            item_choices(items, villain_choice)
        else:
        ...
        if "rake" in items:
            print_pause("You already have that item")
            item_choices(items, villain_choice)
        elif "rake" not in items and len(items) == 1:
            items.append("rake")
            print_pause("You pick up the " + items[1] + "")
            item_choices(items, villain_choice)
        else:
        ...
        if "enchanted blade" in items:
            print_pause("You already have that item")
            item_choices(items, villain_choice)
        elif "enchanted blade" not in items and len(items) == 1:
            items.append("enchanted blade")
            print_pause("You pick up the " + items[1] + "")
            item_choices(items, villain_choice)
        else:
            print_pause("You drop the " + items[1] + "")
...
def win(items, villain_choice):
    print_pause(f"You repel the {villain_choice}'s attack"
                " you unsheath the" + items[1] + "")
    ...
    print_pause(f"As the {villain_choice} is stunned"
                " you smite it with the BLADE!")
    print_pause(f"You have made safe the forest from the"
                f" {villain_choice}, you are a hero!")
    play_again()


def lose(villain_choice):
    print_pause(f"The {villain_choice} kills you Whomp whomp")
    play_again()
